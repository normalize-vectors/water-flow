...
class MyGame(arcade.Window):
    ...
    def randomize_grid(self, grid: arcade.SpriteList):
        """ Randomize the grid """
        for cell in grid:
            pick = random.randrange(2)
            if pick:
                cell.color = (int(255*random.random()), int(255*random.random()), int(255*random.random()))
            else:
    ...
    # def randomize_grid(self, grid: arcade.SpriteList):
        ...
        #     pick = random.randrange(2)
        #     if pick:
        #         cell.alpha = random.randrange(256)
        #     else:
        #         cell.alpha = 255

    def on_update(self, delta_time: float):
        self.randomize_grid(self.grid_sprite_list)
